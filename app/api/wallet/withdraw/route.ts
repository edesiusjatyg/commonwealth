...
export async function POST(
	...
	try {
		...
		const { walletId, amount, category, password, description } =
			validatedData.data;
		...
		if (!wallet) {
			return NextResponse.json(
				{ error: "Wallet not found", message: "Withdrawal failed" },
				{ status: 404 },
			);
		}
		...
		if (amount > balance) {
			return NextResponse.json(
				{ error: "Insufficient balance", message: "Withdrawal failed" },
				{ status: 400 },
			);
		}
